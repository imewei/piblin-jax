...
def copy(arr: Any) -> Any:
    ...
    >>> from quantiq.backend.operations import copy
    >>> arr = jnp.array([1, 2, 3])
    >>> arr_copy = copy(arr)
    """
    if _JAX_AVAILABLE:
...
def concatenate(arrays: Sequence[Any], axis: int = 0) -> Any:
    ...
    --------
    >>> from quantiq.backend import jnp
    >>> from quantiq.backend.operations import concatenate
    >>> arr1 = jnp.array([1, 2])
    >>> arr2 = jnp.array([3, 4])
...
def stack(arrays: Sequence[Any], axis: int = 0) -> Any:
    ...
    --------
    >>> from quantiq.backend import jnp
    >>> from quantiq.backend.operations import stack
...
def reshape(arr: Any, shape: int | Sequence[int]) -> Any:
    ...
    --------
    >>> from quantiq.backend import jnp
    >>> from quantiq.backend.operations import reshape
...
def grad(func: Callable, argnums: int | Sequence[int] = 0, **kwargs) -> Callable:
    ...
    **kwargs
    ...
    Examples
    --------
    >>> from quantiq.backend.operations import grad
    >>> from quantiq.backend import jnp
    >>>
    >>> def loss(x):
    ...     return jnp.sum(x ** 2)
...
def device_put(arr: Any, device: Any | None = None) -> Any:
    ...
    Examples
    --------
    >>> from quantiq.backend.operations import device_put
    >>> from quantiq.backend import jnp
    >>> arr = jnp.array([1, 2, 3])
...
def ensure_array(arr: Any, dtype: Any | None = None) -> Any:
    ...
    Examples
    --------
    >>> from quantiq.backend.operations import ensure_array
...
def astype(arr: Any, dtype: Any) -> Any:
    ...
    Examples
    --------
    >>> from quantiq.backend.operations import astype
    >>> from quantiq.backend import jnp
    >>> arr = jnp.array([1, 2, 3])
