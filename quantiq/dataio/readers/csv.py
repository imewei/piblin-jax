...
class GenericCSVReader:
    ...
    def read(self, filepath: str | Path) -> Measurement:
        ...
        file_metadata = metadata.parse_header_metadata(
            header_lines,
    ...
    def _parse_data_lines(self, data_lines: list[str]) -> np.ndarray:
        ...
        try:
            ...
            raise ValueError(f"Inconsistent number of columns in data: {e}")
    ...
    def _create_datasets(
        ...
        if data_array.shape[1] < 2:
            raise ValueError(
                f"Data must have at least 2 columns (x, y), "
                f"got {data_array.shape[1]}"
            )
