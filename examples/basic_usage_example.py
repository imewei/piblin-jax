...
dataset = OneDimensionalDataset(
    ...
    conditions={"temperature": 25.0, "sample": "Example_A", "noise_level": 0.15},  # °C
    details={
...
print(
    f"   ✓ Data range: x=[{x.min():.2f}, {x.max():.2f}], y=[{y_noisy.min():.2f}, {y_noisy.max():.2f}]"
)
...
print("   ✓ Applied Gaussian smoothing with sigma=2.0")
...
axes[0, 1].plot(
    smoothed_dataset.independent_variable_data,
    smoothed_dataset.dependent_variable_data,
    "b-",
    linewidth=2,
    label="Smoothed (σ=2.0)",
)
...
axes[1, 0].plot(
    processed_dataset.independent_variable_data,
    processed_dataset.dependent_variable_data,
    "g-",
    linewidth=2,
    label="Smoothed + Normalized",
)
